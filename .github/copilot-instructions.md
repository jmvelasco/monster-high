...
- **Double Validation Invariant (Before & After):** Once valid external clearance (PROCEED) is given, you **MUST** execute a two-step evaluation loop:
  1. **Baseline Check:** Execute `npm run validate` from the root _before_ writing any modification to establish repository health. If it fails, **HALT** and report the errors.
...
- **Closed-Loop Pull Request Compliance:** When tasked with creating a Pull Request via GitHub CLI (`gh`), you are **STRICTLY PROHIBITED** from executing a blind `gh pr create` command with a static or raw text string in the `--body` flag. You **MUST** execute a closed-loop compliance sequence:
  1. **Template Inspection:** Read the structural template `.github/pull_request_template.md` using your file reading tools.
  2. **Memory Population:** Adopt the exact Markdown schema of the template into your memory buffer, populate every text section, and verify every required compliance box by dynamically changing `- [ ]` to `- [X]` based strictly on the current session's TDD and validation outcomes.
  3. **Ephemeral Stream Execution:** To prevent disk pollution and the overhead of temporary file lifecycle management, you **MUST** pipe the fully populated compliance markdown directly into the GitHub CLI's standard input. Execute the creation command utilizing the standard input file flag: `gh pr create --base gh-600 -F -`.
