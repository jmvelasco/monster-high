...
We will define an interface that every CLI command must implement.
```
...
A central class in `infrastructure/cli/` that manages the lifecycle of the CLI.
...
```
*Rationale*: Centralizes the usage of `yargs`, making it the only place where the library is imported.
